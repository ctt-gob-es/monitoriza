 ...
 */
@Repository
public interface DailyVipDataTableRepository extends DataTablesRepository<DailyVipMonitorig, Long> {
	 ...
	 */
	@Query("SELECT daily FROM DailyVipMonitorig daily WHERE daily.samplingTime BETWEEN ?1 and ?2")
	DataTablesOutput<DailyVipMonitorig> findDailyVipTimeRange(LocalDateTime min, LocalDateTime max);
}
