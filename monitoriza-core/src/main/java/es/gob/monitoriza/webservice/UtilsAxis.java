...
public final class UtilsAxis {
     ...
     */
    private static int counter = (int) (getRandomDouble() * NumberConstants.NUM5000);
	...
	try {
	    ...
	    LOGGER.error("Ha sido imposible generar el número aleatorio solicitado", e);
	}
	return null;
    }
	...
	if (counter >= NumberConstants.NUM10000) {
	    counter = (int) (getRandomDouble() * NumberConstants.NUM5000);
	...
	if (twoCipherIpRepresentation == null || twoCipherIpRepresentation.isEmpty()) {
		...
		addingResult = (int) (getRandomDouble() * NumberConstants.NUM99);
	...
	return findElementByTagNameAndAttribute(element, tagName, "Id", id);
    }
