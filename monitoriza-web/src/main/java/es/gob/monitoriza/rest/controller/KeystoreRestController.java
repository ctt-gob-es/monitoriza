...
public class KeystoreRestController {
    ...
    public DataTablesOutput<SystemCertificate> users(@Valid DataTablesInput input){
		
		return (DataTablesOutput<SystemCertificate>) certificateService.findAll(input);
...
//    public String loadssl(@RequestParam("file") MultipartFile file) throws IOException {
//    	    	        
//        this.file = file;
    ...
    public @ResponseBody DataTablesOutput<SystemCertificate> saveSsl(@RequestParam("file") MultipartFile file, @RequestParam("alias") String alias) throws IOException {
		...
		if (!error) {
			...
			try {
				...
				// Valida el certificado y lo añade al almacén truststore ssl del sistema
				Keystore ko = keyStoreFacade.storeAlias(alias, cert, null);
